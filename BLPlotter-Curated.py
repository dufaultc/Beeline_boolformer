...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-o", "--output", default=".", help="Output directory for generated plots.\n"
    )
...
def boxplot(opts, evalConfigs, datasets, randValue, resTypeFile, resTypeName):
    ...
    for i, dataset in enumerate(datasets):
        ...
        DF = pd.read_csv(
            str(evalConfig.output_settings.base_dir)
            + "/"
    ...
    f, axes = plt.subplots(
        len(datasets),
        1,
        figsize=(1.5 * max([len(DF.columns) for DF in DFs]), 5 * len(datasets)),
    )
    ...
    for i, dataset in enumerate(datasets):
        ...
        modifiedDF = pd.melt(
            ...
            value_vars=["Boolformer", "PIDC", "GENIE3", "GRNBOOST2"],
        )

        conditions = [
            modifiedDF["dataset"].str.contains("-50"),
            modifiedDF["dataset"].str.contains("-70"),
        ]
        outputs = ["50", "70"]
        modifiedDF["category"] = pd.Series(np.select(conditions, outputs, "0"))
        modifiedDF["category"] = (
            modifiedDF["category"]
            .astype("category")
            .cat.reorder_categories(["0", "50", "70"])
        )
        ...
        ax = sns.lineplot(y=randValue, x=range(-1, len(DF.columns) + 1), ax=subax)
        ...
        ax.legend(
            ...
            loc="upper center",
            ncol=len(modifiedDF["category"].cat.categories),
        )

        ax.set_ylim([0.0, 1])
        ax.set_ylabel(resTypeName, fontsize=22)
        ax.set_title(resTypeName + " values for " + dataset + " Network", fontsize=24)
        plt.tight_layout()
...
def COplot(
    ...
    ax = plt.subplot2grid(
        (2 * maxRows + 3, len(levls)), (0, 0), colspan=2, rowspan=2 * maxRows + 2
    )
                ...
                if shape[levlIdx] not in ["text", "arrow"]:
                    ...
                    if np.isnan(value):
                        ...
                        plt.text(
                            ...
                            color=col,
                            bbox=dict(
                                boxstyle="round", ec=(1, 1, 1, 0), fc=(1, 1, 1, 0)
                            ),
                        )
                        continue
                        ...
                        rangesD = (
                            round(inputDF[levls[levlIdx]][colNames[colIdx]].min(), 3),
                    ...
                    if shape[levlIdx] == "c":
                        ...
                        size = value * 0.8
                        circle1 = patches.Rectangle(
                            (
                                colStart + colIdx + 1 - (size / 2),
                                rowIdx + 1 - (size) / 2,
                            ),
                            width=size,
                            height=size,
                        ...
                        circle1 = patches.FancyBboxPatch(
                            (
                                colStart + colIdx + 1 - (newVal / 2),
                                rowIdx + 1 - (newVal) / 2,
                            ),
                            width=newVal,
                            height=newVal,
                        ...
                        circle1 = patches.Rectangle(
                            (colStart + colIdx + 0.6, rowIdx + 0.65),
                            width=0.75,
                            height=value,
                            facecolor=col,
                            edgecolor="k",
    ...
    for levlIdx in range(len(levls)):
        ...
        ax.imshow(
            np.arange(len(palettes[levlIdx])).reshape(1, len(palettes[levlIdx])),
...
def main():
    ...
    for i, dataset in enumerate(datasets):
        ...
        trueEdgesDF = pd.read_csv(
            str(evalConfig.input_settings.datadir)
            + "/"
            + evalConfig.input_settings.datasets[0]["name"]
            + "/"
        ...
        print(
            "Early Precision of a random predictor for ",
            dataset,
            " network (excluding self loops) is: %.2f" % (randPred),
        )
    ...
    if opts.overview:
        ...
        ResDF = pd.read_csv(
            str(evalConfigs[0].output_settings.base_dir)
            + "/"
        ...
        overviewDF = pd.DataFrame(
            index=algs, columns=pd.MultiIndex.from_tuples(multIndTuple)
        )
            ...
            for j, dataset in enumerate(datasets):
                ...
                for alg in algs:
                    ...
                    if res == "Early Precision Ratio" or res == "AUPRC Ratio":
                        overviewDF.loc[alg][res, dataset] = (
                            ResDF[alg].median() / randPredictor[dataset]
                        )
        ...
        overviewDF = overviewDF.loc[
            overviewDF["AUPRC Ratio"].median(axis="columns").sort_values().index
        ]
        ...
        COplot(
            overviewDF,
            width=3 * len(evalConfigs) + 1,
            height=len(algs) + 3,
            randValues=[1, 0, 0, 0],
            shape=["hm", "hm", "hm", "arrow"],
            palettes=[pale3, pale3h, pale5, pale3r],
            levels=resType,
            rotation=[0, 0, 0, 0],
